                ...
                for(int q=0; q < nsm; q++)
                        for (int r=0; r < nsm; r++)
                                for (int s=0; s < nsm; s++){
                                        ...
                                        cout << p << '\t' << q << '\t' << r << '\t' << s << '\t' << spi_TEI(p,q,r,s) << endl;
                                }
}
...
void CCSD::stor_erramp(int iter){
	err_tai = update_tai - t_ai;
	cout << "tai_stor" << endl;
	cout << t_ai << endl;
	cout << "update tai_stor" << endl;
	cout << update_tai << endl;
	err_taibj = update_taibj - t_aibj;
	for (int i=0; i < nsm_occ; i++){
		for (int a=0; a < nsm_vir; a++){
			int n = (iter-1) % 8;
			errtai_ten(n,i,a) = err_tai(i,a);
			tai_ten(n,i,a) = update_tai(i,a);
			for (int j=0; j < nsm_occ; j++){
				for (int b=0; b < nsm_vir; b++){
					int n = (iter-1) % 8;
					errtaibj_ten(n,i,j,a,b) = err_taibj(i,j,a,b);
					taibj_ten(n,i,j,a,b) = update_taibj(i,j,a,b);
				}
			}
		}
	}
	...
	cout << "err_tai" << endl;
	cout << err_tai << endl;
	cout << "tai matrix from list" << endl;
	cout << update_tai << endl;
}

void CCSD::extra_amp(int numb){
	if (numb > 7)
		numb = 8;
	//cout << "over" << endl;
	B_tai.resize(numb+1,numb+1);
	B_taibj.resize(numb+1,numb+1);
	for (int n=0; n < numb; n++){
		for (int m=0; m < numb; m++){
			...
			for (int i=0; i < nsm_occ; i++){
				for (int a=0; a < nsm_vir; a++){
					//cout << "yes" << endl;
					B_tai(n,m) += errtai_ten(n,i,a)*errtai_ten(m,i,a);
					for (int j=0; j < nsm_occ;j++){
						for( int b=0; b < nsm_vir; b++){
							//cout << "no" << endl;
							B_taibj(n,m) += errtaibj_ten(n,i,j,a,b)*errtaibj_ten(m,i,j,a,b);
						}
	...
	cout << "B for tai" << endl;
	cout << B_tai << endl;
	Eigen::VectorXd C_tai;
	C_tai.setZero(numb+1);
	C_tai(numb) = -1;
	cout << "C for tai" << endl;
	cout << C_tai << endl;
	Eigen::PartialPivLU<Matrix> mat_tai(B_tai);
	Eigen::VectorXd ans_tai;
	ans_tai = mat_tai.solve(C_tai);
	cout << ans_tai << endl;

	cout << "B for taibj" << endl;
	cout << B_taibj << endl;
	
	Eigen::VectorXd C_taibj;
	C_taibj.setZero(numb+1);
	C_taibj(numb) = -1;
	cout << "C for taibj" << endl;
	cout << C_taibj << endl;
	Eigen::PartialPivLU<Matrix> mat_taibj(B_taibj);
	Eigen::VectorXd ans_taibj;
	ans_taibj = mat_taibj.solve(C_taibj);
	cout << ans_taibj << endl;

	ext_tai.setZero(nsm_occ,nsm_vir);
	ext_taibj = Tensor4D (nsm_occ, nsm_occ, nsm_vir, nsm_vir);
	ext_taibj.setZero();
	for (int n=0; n < numb; n++){
		for (int i=0; i < nsm_occ; i++){
			for (int a=0; a < nsm_vir; a++){
				ext_tai(i,a) += ans_tai(n)*tai_ten(n,i,a);
				for (int j=0; j < nsm_occ; j++){
					for (int b=0; b < nsm_vir; b++){
						ext_taibj(i,j,a,b) += ans_taibj(n)*taibj_ten(n,i,j,a,b);
					}
				}
			}
		}
