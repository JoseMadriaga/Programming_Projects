...
//Iteration process of updating the t amplitudes from corresponding intermediates and (effective) taus until the correlation energy converges within a threshold
	...
	CC.tai_ten = Tensor3D (8,CC.nsm_occ, CC.nsm_vir);
	CC.taibj_ten = Tensor5D (8,CC.nsm_occ, CC.nsm_occ, CC.nsm_vir, CC.nsm_vir);
	CC.errtai_ten = Tensor3D (8,CC.nsm_occ,CC.nsm_vir);
	CC.errtaibj_ten = Tensor5D (8,CC.nsm_occ,CC.nsm_occ, CC.nsm_vir, CC.nsm_vir);
	...
	for (int i=1; i < maxiter; i++){
		...
		if (i > 2){
			...
			CC.extra_amp(i-1);
			CC.t_ai = CC.ext_tai;
			CC.t_aibj = CC.ext_taibj;
			cout << "Replacing t_ai" << endl;
			cout << CC.t_ai << endl;
		}
		CC.tau_and_eff(CC.t_ai, CC.t_aibj, CC.tau_eff, CC.tau);
		cout << "t_ai" << endl;
